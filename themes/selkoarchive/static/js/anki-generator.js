...
class AnkiGenerator {
  ...
  extractMetadata() {
    ...
    const dateMatch = localUrl.match(/\/(\d{4})\/(\d{2})\/(\d{2})\//);
    const date = dateMatch ? `${dateMatch[1]}-${dateMatch[2]}-${dateMatch[3]}` : null;

    // Build production URL from the date
    const productionBaseUrl = 'https://hiandrewquinn.github.io/selkouutiset-archive';
    const articleUrl = dateMatch
      ? `${productionBaseUrl}/${dateMatch[1]}/${dateMatch[2]}/${dateMatch[3]}/`
      : productionBaseUrl;
  ...
  /**
   * Format card header for the front of the card
   * Shows article title (linked), date, and location context
   */
  formatCardHeader(metadata, context = {}) {
    const parts = [];

    parts.push('<div style="margin-bottom: 1em; padding-bottom: 0.75em; border-bottom: 1px solid #ccc; font-size: 0.8em; color: #555;">');

    // Article title (linked to the archive)
    if (metadata.articleTitle) {
      parts.push('<div style="margin-bottom: 0.4em;">');
      parts.push(`<a href="${metadata.url}" style="color: #a8763e; text-decoration: none; font-weight: bold;">${this.escapeHtml(metadata.articleTitle)}</a>`);
      parts.push('</div>');
    }

    // Date and location on the same line
    const locationParts = [];
    if (metadata.date) {
      locationParts.push(metadata.date);
    }

    // Context: Section, Paragraph, Sentence
    if (context.section && context.sectionNumber) {
      locationParts.push(`Section: ${this.escapeHtml(context.section)} (#${context.sectionNumber})`);
    } else if (context.paragraphNumber) {
      let loc = `Paragraph ${context.paragraphNumber}`;
      if (context.sentenceNumber) {
        loc += `, Sentence ${context.sentenceNumber}`;
      }
      locationParts.push(loc);
    }

    if (locationParts.length > 0) {
      parts.push('<div style="color: #777;">');
  ...
  /**
   * Format card footer for the back of the card
   * Shows attribution and links
   */
  formatCardFooter(metadata) {
    const parts = [];
    ...
    parts.push('This Anki card was generated by ');
    parts.push(`<a href="${metadata.baseUrl}" style="color: #a8763e;">Andrew's Selkouutiset Archive</a>.`);
    parts.push('</div>');

    parts.push('<div style="margin-bottom: 0.5em;">');
    parts.push('You can find other tools Andrew Quinn has built for learning Finnish at ');
    parts.push('<a href="https://finnish.andrew-quinn.me/" style="color: #a8763e;">https://finnish.andrew-quinn.me/</a>.');
    parts.push('</div>');

    // Footer with GitHub link and generation date
  ...
  cardsToTSV(cards, metadata = {}) {
    ...
    // Add cards with header on front, footer on back
    cards.forEach(card => {
      // Build front: header + content
      const cardHeader = this.formatCardHeader(pageMetadata, card.context || {});
      const frontContent = card.front.replace(/\t/g, ' ').replace(/\n/g, '<br>');
      const front = cardHeader + frontContent;

      // Build back: content + footer
      const backContent = card.back.replace(/\t/g, ' ').replace(/\n/g, '<br>');
      const cardFooter = this.formatCardFooter(pageMetadata);
      const back = backContent + cardFooter;

      const tags = metadata.tags || 'andrews-selkouutiset-archive';
  ...
  async generate(level) {
    try {
      ...
      const metadata = {
        title: title,
        url: url,
        tags: `andrews-selkouutiset-archive ${this.currentLang}-${this.otherLang} ${level} ${date}`
      };
